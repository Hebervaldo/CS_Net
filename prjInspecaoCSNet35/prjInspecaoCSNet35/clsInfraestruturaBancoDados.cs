    ...
    public partial class clsInfraestruturaBancoDados : IDisposable
    {
        ...
        public int mtdObterNumeroColunaLista(string Coluna)
        {
            lock (LockInfraestruturaBancoDados)
            {
                ...
                List<object> lstObterCabecalhoColunas = mtdObterCabecalhoColunasLista();
                int saida = -1;

                for (int contador = 0; contador <= lstObterCabecalhoColunas.Count - 1; contador++)
                {
                    if (Coluna == lstObterCabecalhoColunas[contador].ToString())
                    {
                        saida = contador;
                    }
                }
